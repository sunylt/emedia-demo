...
function getTicket(roomId) {
  return fetch("".concat(serverUrl, "/emedia/enter_room?app_id=").concat(appId, "&&user_sig=").concat(userSig, "&room_id=").concat(roomId, "&user_id=").concat(username)).then(function (res) {
    return res.json();
  });
}
  ...
  getTicket(roomId).then(function (result) {
    ...
    service.join(function () {
      ...
      service.openUserMedia(pubS).then(function () {
        // 推流
        service.push(pubS, function () {}, function () {});
      });
    });
  });
} // 获取并保存用户签名
...
$("#joinRoom").addEventListener("click", function () {
  // 自己起个roomId
  var roomId = document.querySelector("input").value;
  joinRtcRoom(roomId);
}); // 自己退
